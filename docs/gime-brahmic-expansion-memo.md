...
| varga | 子音 | LS 方向 |
|---|---|---|
| कवर्ग (軟口蓋) | क ख ग घ ङ | **LS ↑** |
| चवर्ग (口蓋) | च छ ज झ ञ | **LS ←** |
| टवर्ग (そり舌) | ट ठ ड ढ ण | **LS →** |
| तवर्ग (歯) | त थ द ध न | **LS ↓** |
| पवर्ग (唇) | प फ ब भ म | **LS 中立** |

各 varga 内は **「無気無声 / 有気無声 / 無気有声 / 有気有声 / 鼻音」** の
5 行対称構造。これを:

- **D-pad 4 方向 = 4 stops**（↓ 無気無声、↑ 有気有声、← 有気無声、→ 無気有声）
- **LB = 鼻音**（各 varga に 1 個のみなので D-pad 要らず、単独キー直送）

で割り付ける。cardinal 直結（「この方向 = この子音」の 1 対 1）方式を
採用し、**1 子音 = 1 アクション**を死守する（conjunct 連鎖の流暢さ優先）。
...
- **face buttons 4** = a, i, u, e（最頻 4 短母音）
- **RS ←** = o
- **RS ↓** = ṛ（低頻度だが Sanskrit で必須）
- **RS →** = **長母音 post-shift**（直前母音を長母音に昇格: ā / ī / ū / ṝ / ai / au）
