...
| 5 | **これから**: いま新配列は薙刀式だけ。NICOLA・月配列・AZIK などの定義は用意済み(ページを足すだけ)。配列は JSON で書ける形式 →「新配列はとりあえずへちま言語ラボで試せる」場所へ。**載せてほしい配列があれば教えてください** | keymap-engine v1.4.0 は逐次/時間窓/相互シフト全対応(裏付け済み) |
...
### GIF ① ふだんどおりのローマ字(トップ `/`、20〜25 秒)

準備: OS IME オフ(英数直接入力)/ トップを開く / 「クリア」→「学習リセット」
