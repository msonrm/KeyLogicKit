...
グローバル名 `GamepadEngine`、UMD（`<script>` / `importScripts` / `require`）。約 29KB（min 14KB）。
...
| `"y"`（code `KeyY`） + `ctrlKey` | **RT + Start**（やり直し） | false（セッションは修飾コンボを透過） | false → ホストが文書 redo（v1.7.0+） |
...
（`applyFlickHostKey` が `Ctrl+BS` を文書 undo に落とす経路をそのまま流用できる）。**RT+Start（v1.7.0+）は
その対称形の「やり直し」**で、セッションに対応機能が無いため常に透過される → ホストが `Ctrl+y` を
**文書 redo** に配線する（物理キーボードの Ctrl+Y / Ctrl+Shift+Z と同じ経路を流用すればよい）。
...
- **LT** 単押し = 拗音後置シフト（か→きゃ・あ→ぁ、対象外は「っ」追加）、**LT+RT** = っ、
  **RT** 単押し = ん。**RT 連打**（v1.4.0+）= ん→を→んを→ん の循環（前段を replace で差し替え。
  タップ間に別出力が入るとサイクルは切れる）
- **右スティック**: ↑ = 濁点/半濁点トグル（v1.4.0+ = 末尾が拗音の小書き ゃゅょ なら直前の基字に適用:
  きゃ→ぎゃ）、→ = 長音「ー」、← = バックスペース、
  ↓ = 、→。→空白（400ms ダブルタップで昇格、3 連で確定）
- **左スティック**（v1.1.0+）: ↓ = 合成中は変換開始/次候補（Space）・**未入力時はカーソル下**
  （ArrowDown、v1.3.0+。空白の誤入力回避）、↑ = 前候補/カーソル上、←→ = 文節移動/カーソル移動。
  **RT + ←→ = 文節伸縮**（Shift+←→、v1.2.0+）。**非合成時の RT + 上下左右 = 範囲選択**（Shift+矢印、
  v1.5.0+）。すべて `key` op として emit し、合成中は hechima セッションが状態で解釈、非合成は feed が
  false を返してホストのエディタ操作へ落ちる（§3 の表）。合成中かどうかは `getComposingTail()` の空判定で見る。
  **非合成の LS カーソル移動・範囲選択はキーリピートする**（v1.6.0+。倒し続けで初回 400ms → 以後 85ms ごと
  同じ op を再 emit。合成中の変換/候補/文節移動はエッジのみ）
- **LS 押込み** = 確定/改行、**RS 押込み** = キャンセル、**Start** = 確定アンドゥ（戻す、v1.2.0+）、
  **RT + Start** = やり直し（redo、v1.7.0+。`Ctrl+y` を emit。RT は「使用済み」に印を付けるので
  リリース時の「ん」は出ない）

> **範囲選択・範囲削除・再変換はホストの責務**（v1.5.0）: エンジンは選択状態を持たず、RT+LS で
> **Shift+矢印**を emit するところまで。ホストは (1) Shift+矢印を編集領域の選択拡張（`sel.modify("extend", …)`）
> として処理、(2) Backspace を「選択があれば範囲削除」に、(3) 選択があるときの確定アンドゥ（Ctrl+BS）を
> 再変換へ振り替える、を担う（へちま言語ラボの `applyFlickHostKey` / gamepad `onOp` が実装例）。
...
- **OS ソフトウェアキーボードの抑止**: ゲームパッド入力は「エディタにフォーカスがあるときだけ
  有効」にするのが定石（物理キーボードと同じ前提）。だがタッチ端末で編集領域（contenteditable 等）を
  タップしてフォーカスすると OS の画面キーボードが出てしまう。編集領域に `inputmode="none"` を
  指定して抑止する（hechima は OS IME 非依存なので、フォーカスは許しつつ画面キーボードだけ封じられる）。
  iOS/iPadOS Safari の実機確認を必ず行う（宣言だけで済まない場合は `readonly` + caret 自前管理への
  フォールバックを検討）。
- **物理キーボードとは非排他**: `inputmode="none"` は画面キーボードのヒントであって物理キーボードの
  `keydown` には影響しない。GamepadEngine は keyboard イベントを介さないので、ゲームパッドと
  物理キーボードは同一エディタで併用できる。
- **フリックとの共存**: 同じホストで FlickEngine も出すなら、どちらのモードでも `inputmode="none"` を
  維持すること。フリックの有効/無効切替で属性を消すと、ゲームパッドが有効なのに画面キーボードが
  復活する。「ゲームパッド有効 or フリック有効なら none」で一元管理するのが安全（ラボサイトは
  両者を別ページに分離してこの競合を回避している）。
