...
- **薙刀式 ラスボス戦の本検証**: 実機 (iPad) で 159 chord を一通り試す。
  特に 3 キー chord (外来音 でぃ / てぃ / ぐぁ / どぅ 等) が想定通り
  「2 キー部分 emit → 3 キー目で BS 巻き戻し + 再 emit」 のフローで動くか。
  小書き `ぃ` を含む 「でぃ」 系は iPad ANSI JIS かな で表現不可能な可能性
  あり、要実機確認。
- **`switchToEnglish` + `englishLookupTable`** 対応 (薙刀式 の英語モード切替)。
