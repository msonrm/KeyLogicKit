...
- 関連: [`docs/keymap-engine-embedding.md`](keymap-engine-embedding.md) /
  [`docs/hechima-protocol.md`](hechima-protocol.md)（へちま蔓 v0 = worker RPC 仕様） /
...
- v0.4.0+ は **`hechima-worker.js`**（hechima-wasm を動かす Worker 本体、IIFE。min 版も）を
  ...
  （へちま蔓 v0。仕様は [`docs/hechima-protocol.md`](hechima-protocol.md)）。
...
| `HECHIMA_PROTOCOL_VERSION`（v0.4.0+） | へちま蔓プロトコル版数（[`hechima-protocol.md`](hechima-protocol.md)） |
...
<script>
  // 実変換: hechima-worker（へちま蔓 v0）に接続すると convert/resize が cb 形で得られる。
...
| `deleteBack` | **よみに戻す**（v0.8.1+、標準 IME 準拠。旧 = 全クリア） | **engine 既定**（composingKana 末尾削除） | `cb.hostKey('Backspace')` |
...
| `confirm`（薙刀式 M+V 等） | **結合確定**（v0.3.0+） | **よみのまま即確定**（v0.12.0+。変換に流さない） | engine 既定（空振り） |
| `insertAndConfirm`（space+M=。等） | **現候補確定 + テキスト確定**（v0.3.0+） | **よみ + テキストを即確定**（v0.12.0+） | **テキストを直接確定**（v0.12.0+） |

**v0.12.0 の変更**: 合成中の `confirm` / `insertAndConfirm` は従来 convert と同じく
「確定かな → 変換開始（候補表示）」に化けていたが、本家薙刀式 v18 の VM = Enter・
「、{Enter}」（句読点 + 即確定）に合わせて**無変換の直接コミット**に修正した。
実装はワンショットフラグ: `handleEngineAction` が立て、直後の `pumpEngine` が確定かなを
`startConvert` に流す代わりに `commit()` する。golden は `confirm_direct.json`。
...
- **ナビキー**（Enter / Escape / Backspace / ←→↑↓ — chord 語彙に参加しない実キー）→
  セッションが直接処理（結合確定・**BS/Escape = よみに戻す**（v0.8.1+）・文節移動・候補送り・
  **Shift+←→ = 文節伸縮**）。戻した**よみ復帰状態**（よみは session 保持・engine は空）では
  BS / U = 末尾削除、Enter / M+V = よみのまま確定、Escape = 取消、space 単打 = 再変換、
  かな追加 = よみに連結して合成継続（表示は session よみ + engine 合成の連結）
