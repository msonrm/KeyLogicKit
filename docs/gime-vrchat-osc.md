...
> **【アーカイブ / 撤去済み】** GIME の VRChat 連携（OSC 送信 / バブル表示 / 二段送信翻訳）は 2026-07 に撤去し、GIME を「ゲームパッドによるアジア圏言語 IME」に純化した。撤去理由: 体験が振るわず、IME 再定義という核だけ残す判断。実装コードは git tag `gime-vrchat-impl-archive`（撤去前 HEAD = commit b8379cb）に保存。本ドキュメントは設計・プロトコル・運用知見の記録として残す。
...
### 入力中アバターを「考え中ポーズ」にしたい

棒立ちを避けたい無言勢向けに、**composing 開始/終了エッジで任意の
avatar parameter を OSC で叩く機能**があります（iOS / Android 両対応）。

VRChat 設定画面の **「入力中アクション」** セクション:

- **トグル**: 有効化
- **アドレス**: 例 `/avatar/parameters/VRCEmote`
- **値の型**: `int` / `float` / `bool`
- **開始時の値**: composing 非空に入った瞬間に送る値（例 `7`）
- **終了時の値**: commit or クリアで戻す値（例 `0`）
- **プリセット**: VRCEmote=7 (sadness) — SDK デフォルトテンプレのままの
  アバターなら無改造で試せるが、本来の「悲しみ」エモートなので見た目は
  少し違和感あり

推奨は **自分のアバター専用の pose / idle 系 parameter を探して当てる**
方法。以下のいずれかで見つけられます:

1. VRChat 内の **Action Menu → Options → OSC → OSC Debug** を開いて、
   現在のアバターが公開している parameter 一覧を眺める
2. Expression Menu のトグルを手動で切替えながら OSC Debug を見て、
   「このトグルがこの parameter を変えている」を特定
3. VRCFaceTracking 対応アバターなら `/avatar/parameters/v2/JawOpen`
   を `0.2` くらいにすると「口半開きで考えてる顔」に寄せられる（float）

「タイピングインジケータ」「確定時のみ送信」とは独立して動作するため、
3 つの設定を自由に組み合わせられます。
...
- 設定トグル:
  ...
  - `customTypingEnabled` — 入力中アクション。composing の開始/終了
    エッジで任意の avatar parameter を `int` / `float` / `bool` で送る
    （address / valueType / startValue / endValue をセットで設定）
  - `translationTarget` — 二段送信翻訳のターゲット言語
    （`OFF` / `English` / `한국어` / `中文(简体)` / `中文(繁體・台湾)`）
  - `translationOverwriteDelayMs` — 翻訳上書きまでの最低遅延 (ms)

## 二段送信翻訳（オプション）

LS で日本語を `/chatbox/input` に送信したあと、裏で翻訳して **notification=false** で
同じ chatbox を上書きする機能。VRChat 側では「日本語が音付きで表示 → スッと
翻訳に置き換わる」挙動になり、待ち時間が UX 上の演出に変わる。

### 仕組み

- 翻訳は **ML Kit On-Device Translation**（端末内で完結、第三者サーバー不経由）
- 初回はモデル ~30MB を on-demand ダウンロード（デフォルト WiFi 限定）
- 翻訳後の追加送信は notification=false なので相手側に通知音は鳴らない
- 翻訳に失敗したら日本語のまま残る無音フォールバック

### 設定方法

1. VRChat OSC 設定画面の「送信後の自動翻訳（二段送信）」セクション
2. **ターゲット**: `OFF` / `English` / `한국어` / `中文(简体)` / `中文(繁體・台湾)` から選択
3. **WiFi のみでモデル DL**: モバイル回線消費を防ぐ（デフォルト ON）
4. **上書きまでの遅延**: 即時 / 1s / 1.5s / 2s / 3s
   - 翻訳が終わってもこの時間が経過するまで上書きを待つ。原文を読む間が
     確保され、デモ動画でも視認性が上がる
   - デフォルト 1.5s
5. **「モデルを事前ダウンロード + テスト」ボタン**: 初回利用前に DL を済ませて
   おくと、本番送信時に余計な遅延が出ない

### 中文の二択について

ML Kit Translate は中文を「簡体のみ」しかモデルを持たないため、繁體（台湾）
ターゲットを選ぶと、**OpenCC ベースの語彙置換**が後段で走る:
...
辞書は OpenCC4j（Apache 2.0、純 JVM、JNI 不要）に同梱される 49,000 件の
S→T フレーズマップを使用。

### iOS (GiME) について

iOS 版への移植は今後対応予定。現時点では Android 版のみ。

## プライバシー

OSC 送信先はユーザーが設定した IP:port のみ。第三者サーバーへのデータ
送信は一切ありません（翻訳もすべて端末内で完結）。詳細は
[プライバシーポリシー](gime-android-privacy-policy.md) 参照。
